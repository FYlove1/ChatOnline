...
std::vector<std::string> Sql_Manager::GetFriends(const std::string &Account) const {
    std::vector<std::string> friends;
    std::lock_guard<std::mutex> lock(mtx); // 保持线程安全

    // 准备查询好友的SQL语句
    std::string sql = "SELECT FriendAccount FROM ChatServer.UserFriend WHERE Account = ?";
    MYSQL_STMT* stmt = prepareStatement(sql);
    if (!stmt) {
        std::cerr << "准备好友查询SQL失败" << std::endl;
        return friends;
    }
    ...
    bind_param[0].buffer = (void*)Account.c_str();
    bind_param[0].buffer_length = Account.length();
    ...
    // 绑定结果集
    char friendAccount[21] = {0}; // varchar(20) + 终止符
    unsigned long length = 0;
    MYSQL_BIND bind_result[1];
    std::memset(bind_result, 0, sizeof(bind_result));
    bind_result[0].buffer_type = MYSQL_TYPE_STRING;
    bind_result[0].buffer = friendAccount;
    bind_result[0].buffer_length = sizeof(friendAccount) - 1; // 保留终止符位置
    ...
    while (!mysql_stmt_fetch(stmt)) {
        friendAccount[length] = '\0'; // 确保字符串终止
        friends.emplace_back(friendAccount);
    }
